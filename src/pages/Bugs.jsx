...
const Bugs = () => {
  ...
  const allTags = useMemo(() => {
    ...
    bugs.forEach(item => {
      if (item.severity) tags.add(item.severity);
  ...
  const filteredBugs = bugs.filter(bug => {
    const matchesSearch = bug.title.toLowerCase().includes(searchQuery.toLowerCase()) || bug.severity.toLowerCase().includes(searchQuery.toLowerCase());
  ...
  const handleExportCSV = () => {
    ...
    link.href = url;
    link.setAttribute('download', `bugs_log_${new Date().toISOString().split('T')[0]}.csv`);
  ...
  const handleDrop = (e, quadrant) => {
    ...
    updateEisenhower(taskId, 'bugs', quadrant);
  };

  const handleSort = () => {
    sortDataByEisenhower('bugs');
      ...
      <div className="w-1/3 xl:w-1/4 border-r border-[var(--border-color)] flex flex-col bg-[var(--bg-main)]">
        <div className="p-6 border-b border-[var(--border-color)] bg-[var(--bg-card)]">
          <div className="flex justify-between items-center mb-4">
            <h1 className="text-xl font-black tracking-tight uppercase">Bugs Log</h1>
          ...
          <div className="space-y-4">
            <div className="relative">
              ...
              <input type="text" placeholder="Filter bugs..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="w-full pl-9 pr-4 py-2 bg-[var(--bg-main)] border border-[var(--border-color)] rounded-xl text-sm" />
            </div>
        ...
        <div className="flex-1 overflow-y-auto p-4 space-y-3">
          {filteredBugs.map(item => (
            <div 
              ...
              className={`p-5 rounded-2xl border cursor-pointer transition-all ${selectedItem?.id === item.id && !isEisenhower ? 'border-red-500 bg-[var(--bg-card)] shadow-xl translate-x-1' : 'border-transparent bg-[var(--bg-card)] hover:border-red-500/30'} ${isEisenhower ? 'cursor-grab active:cursor-grabbing' : ''}`}
            >
              <div className="flex justify-between items-start mb-3">
                <span className={`px-2 py-0.5 rounded-md text-[10px] font-black uppercase tracking-widest border ${getSeverityColor(item.severity)}`}>
                  {item.severity}
                </span>
                {item.eisenhower && <span className="w-2 h-2 rounded-full bg-red-500 shadow-[0_0_8px_rgba(239,68,68,0.8)]" />}
              </div>
      ...
      <div className="flex-1 bg-[var(--bg-card)] flex flex-col relative overflow-hidden">
        <AnimatePresence mode="wait">
          {isEisenhower ? (
                ...
                {[
                  ...
                  >
                    ...
                    <div className="flex-1 overflow-y-auto space-y-2">
                      {bugs.filter(b => b.eisenhower === q.id).map(b => (
                        <div key={b.id} className="bg-[var(--bg-card)] p-3 rounded-xl border border-[var(--border-color)] text-xs font-bold shadow-sm">
                          {b.title}
                        </div>
                      ))}
                    </div>
            ...
            <motion.div key={selectedItem.id} initial={{ opacity: 0, x: 20 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -20 }} className="flex-1 flex flex-col h-full">
              <div className="p-10 border-b border-[var(--border-color)]">
                <div className="flex justify-between items-start">
                  ...
                  {isPrivileged && (
                    <button 
                      onClick={() => addToDevList(selectedItem, 'Bug')} 
                      disabled={isPushed}
            ...
            <div className="flex-1 flex items-center justify-center text-[var(--text-muted)]">Select a bug</div>
          )}
        </AnimatePresence>
      </div>
    </div>
